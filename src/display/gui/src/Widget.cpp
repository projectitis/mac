...
namespace mac {
    ...
    void Widget::title( char* title, char* acronym ) {
        ...
        if ( title ) {
            int len = min( MAC_GUI_MAX_TITLE_LEN, strlen( (const char*)title ) );
            this->_title = new char[len + 1];
            strncpy( this->_title, (const char*)title, len );
        }
        if ( acronym ) {
            this->_acronym = new char[MAC_GUI_MAX_ACRONYM_LEN + 1];
            strncpy( this->_acronym, (const char*)acronym, MAC_GUI_MAX_ACRONYM_LEN );
        }
        else if ( title ) {
            // Create an acronym from the title. CamelCase, ignores whitespace. Examples:
            // My Widget` = MyW, Settings = Set, M8tes = M8t etc
            this->_acronym = new char[MAC_GUI_MAX_ACRONYM_LEN + 1];
            int count = MAC_GUI_MAX_ACRONYM_LEN;
            int soffset = 0;
            int doffset = 0;
            bool caps = true;
            while ( doffset < count ) {
